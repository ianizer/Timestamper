...
def list_years():
    numYears = datetime.now().year - datetime.fromtimestamp(0).year # issue: if current date is before epoch, then negative value occurs.  however this doesn't matter because times before the epoch are impossible to display on discord, without utilizing integer overflow.

    year_list = []

    for i in range(numYears + 50):
        year_list.append((datetime.fromtimestamp(0).year + i).__str__())
...
class Timestamper:
    def __init__(self):
        ...
        self.year_input = ctk.CTkComboBox(self.gui, values=list_years())
        self.year_input.grid(row=1, column=0, sticky="w", padx=50, pady=10) # place on grid (make visible)

        self.copy_button = ctk.CTkButton(self.gui, text="Copy to Clipboard", command=self.bt_copy_to_clipboard)
        self.copy_button.grid(row=2, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
